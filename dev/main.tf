...
module "front" {
  ...
  vpc_id = module.network.vpc_id
  security_group_id = module.security_groups.front_security_group_id ## 보안그룹
  ...
  back_dns = module.back.back_dns ## Back 서버 주소
  iam_code_deploy_arn = module.security_groups.iam_code_deploy_arn # CodeDeploy IAM
}
# Springboot
module "back" {
  source = "../modules/instance/backend"
  vpc_id = module.network.vpc_id
  security_group_id = module.security_groups.back_security_group_id
  alb_security_group_id = module.security_groups.alb_security_group_id
